...
const updateStockDebt = `-- name: UpdateStockDebt :exec
UPDATE debts
SET 
...
type UpdateStockDebtParams struct {
	ID        uuid.UUID
	PricePaid float64
}

func (q *Queries) UpdateStockDebt(ctx context.Context, arg UpdateStockDebtParams) error {
	_, err := q.db.ExecContext(ctx, updateStockDebt, arg.ID, arg.PricePaid)
	return err
}
